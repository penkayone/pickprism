...
## Осиротевший helper `pickprism_user_hue`

**Где:** `inc/template-helpers.php`

**Что есть сейчас.** После удаления sidebar-authors (2026-04-29) функция `pickprism_user_hue` нигде не вызывается. Оставлена в коде как публичный helper на случай повторного появления автор-блоков (related authors, fallback-аватары).

**Что сделать.** Либо удалить, если в обозримом будущем фичи с авторами не планируются, либо оставить осознанно — но тогда хотя бы переиспользовать в `template-parts/author-block.php` для fallback-аватара (сейчас он считает hue вручную / иначе).

**Оценка:** 5 минут на удаление, 15 минут на переиспользование.
**Приоритет:** low.
